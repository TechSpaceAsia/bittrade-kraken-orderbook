...
def base_order_book():
    ...
    compute_order_book(order_book, SAMPLE_BOOK_SNAPSHOT)
    return order_book
...
def test_update_existing_ask_at_given_price():
    message = [544, {"a": [[
        "333.03000",
        "81.02307992",
...
def test_create_ask_lower_than_lowest():
    message = [544, {"a": [["332.01", "56.322", "1620364196.038137"]], "c": "2060521393"}, "book-25", "XRP/USD"]
    order_book = base_order_book()
    old_bids = deepcopy(order_book.bids)
    compute_order_book(order_book, message)
    assert order_book.bids == old_bids, 'Bids should not change'
...
def test_create_multiple_asks_in_the_middle():
    """This also tests that we are able to ignore 'republish' """
    message = [544, {"a": [[
        "332.16000",
        "8.123456",
        "1620209397.284868",
...
def test_remove_one_ask_and_add_one_at_the_back():
    message = [544, {"a": [[
        "332.11000",
        "0.000",
        "1620209397.284868"
...
def test_update_two_sides():
    # At least in theory, Kraken doesn't send two 'c' keys, only one together with 'b'
    message = [544, {"a": [[
        "332.16000",
        "8.123456",
        "1620209397.284868"
